...
3. **HA add-on** (`aventproxy-bridge-addon/`) — Docker container packaging the Go bridge for the HA
   add-on store. One card only. A bridge older than the integration can miss config fields, so keep
   the two in step.
4. **LAN path** (`avent-webrtc-bridge/pkg/lan/`) — cloudless streaming: Tuya LAN discovery and
   session on 6668, protocol-302 signalling, ICE, then KCP/AES-CBC media. Tried before the cloud
   path and falls back to it.
...
- Go 1.23, static build (`CGO_ENABLED=0`). CI gates `gofmt -l` and runs `go test ./...`, so format
  ...
  `avent-webrtc-bridge/`
- Reverse-engineering notes and methodology live in `README.md` ("Protocol reference")
